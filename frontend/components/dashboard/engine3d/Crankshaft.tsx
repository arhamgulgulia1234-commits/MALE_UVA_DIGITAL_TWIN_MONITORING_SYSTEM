 ...
 *
 * These are two *separately selectable* parts sharing one file, and they are rendered as
 * sibling groups rather than nested ones so a click on the shaft cannot also fall through
 * to the case around it. They are genuinely different subsystems: the case and sump are
 * scored under lubrication and simulated by `lubrication_model.py`, while the rotating
 * assembly belongs to `engine_model.py`.
 */
export function Crankshaft({ xray }: { xray: boolean }) {
  ...
  const caseSel = usePartInteraction("crankcase-oil");
  const shaftSel = usePartInteraction("crankshaft");
  ...
  const caseMat = partMaterial(caseSel.visual, xray, 0.3);
  const shaftMat = partMaterial(shaftSel.visual, xray, 0.3);

  return (
    <>
      {/* --- crankcase & oil system ------------------------------------- */}
      <group {...caseSel.handlers}>
        ...
        <mesh>
          ...
          <meshStandardMaterial
            ...
            transparent={caseMat.transparent}
            opacity={caseMat.opacity}
          />
          <PartOutline visual={caseSel.visual} />
        </mesh>
        ...
        <mesh
          position={[LAYOUT.crankcase.length / 2 + 0.12, 0, 0]}
      ...
      {/* --- crankshaft & propeller hub ---------------------------------- */}
      <group {...shaftSel.handlers}>
        ...
        <group ref={shaft}>
          <mesh rotation={[0, 0, Math.PI / 2]}>
            ...
            <meshStandardMaterial
              ...
              roughness={0.22}
              onUpdate={refreshMaterial}
              transparent={shaftMat.transparent}
              opacity={shaftMat.opacity}
            />
            <PartOutline visual={shaftSel.visual} />
          </mesh>
